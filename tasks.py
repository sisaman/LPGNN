...
class Task:
    ...
    def prepare_data(self):
        ...
        if self.feature == 'priv':
            return self.one_bit_response(data, epsilon=self.epsilon, priv_dim=self.priv_dim)
...
class ErrorEstimation(Task):
    ...
    def prepare_data(self):
        return self.one_bit_response(self.dataset[0], epsilon=self.epsilon, priv_dim=self.priv_dim)

    @torch.no_grad()
    def run(self):
        data = self.prepare_data()
        model = self.init_model(data)
